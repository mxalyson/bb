...
class LiveTradingBot:
    ...
    def open_position(self, current_candle, signal, confidence, risk_used: float = None, conf_level: str = "STANDARD", sl_mult: float = None, tp_mult: float = None):
        ...
        # 🔒 PROTEÇÃO CRÍTICA: Evitar trades duplicados
        # Bybit One-Way mode: abrir outro trade na mesma direção AUMENTA a posição existente
        # Isso quebra o gerenciamento de SL/TP individual ❌
        if self.position:
            existing_dir = self.position['direction']
            new_dir = 'long' if signal == 1 else 'short'
            display.warning(f"⚠️ JÁ EXISTE POSIÇÃO {existing_dir.upper()} ABERTA!")
            display.warning(f"⚠️ Tentando abrir {new_dir.upper()} - BLOQUEADO para evitar aumentar posição")
            display.warning(f"💡 Aguarde fechar a posição atual antes de abrir nova")
            return
    ...
    def run(self):
        """
        ⚡ LOOP OTIMIZADO - Opera no fechamento do candle (igual backtest 2.py)

        - Usa iloc[-2] (último candle FECHADO)
        - Só analisa candles NOVOS (evita duplicação)
        - Features estáveis (calculadas em candle completo)
        - Espera inteligente (próximo candle)
        """
        display.info("="*80)
        display.info("🚀 BOT INICIADO - OPERA NO FECHAMENTO DO CANDLE")
        display.info("="*80)
        display.info(f"Symbol: {self.symbol}")
        display.info(f"Timeframe: {self.timeframe}m")
            ...
            while True:
                ...
                try:
                    ...
                    if not self.position:
                        ...
                        # ⚡ CRÍTICO: Marcar candle como analisado (antes de abrir trade)
                        # Evita processar o mesmo candle múltiplas vezes
                        ...
                        if signal != 0 and passes:
                            ...
                            display.info(f"⏭️ Sinal FILTRADO - aguardando próximo candle")
